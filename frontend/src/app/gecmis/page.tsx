...
export default function GecmisPage() {
  ...
  return (
    <div className="min-h-screen px-3 pt-4" style={{ paddingBottom: 96, maxWidth: 700, margin: '0 auto' }}>
      ...
      <p className="text-sm mb-5" style={{ color: '#B0BEC5' }}>Son cevapladığın sorular</p>

      {loading && items.length === 0 ? (
        <div className="text-center py-10" style={{ color: '#B0BEC5' }}>Yükleniyor...</div>
      ) : items.length === 0 ? (
        <div className="glass p-8 text-center" style={{ borderRadius: 14, color: '#607D8B' }}>Henüz cevapladığın soru yok.</div>
      ) : (
            ...
            return (
              <div key={i} className="glass p-3 flex items-start gap-3" style={{ borderRadius: 12 }}>
                ...
                <div className="flex-1 min-w-0">
                  <div className="text-sm font-semibold" style={{ color: '#fff' }}>{it.question_text}</div>
                  ...
                  <div className="text-xs mt-1" style={{ color: '#607D8B' }}>
