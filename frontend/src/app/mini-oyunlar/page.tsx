...
export default function MiniOyunlarPage() {
  return (
    <div className="min-h-screen px-3 pt-4" style={{ paddingBottom: 96, maxWidth: 700, margin: '0 auto' }}>
      ...
      <p className="text-sm mb-5" style={{ color: '#B0BEC5' }}>Farklı bellek ve zeka oyunları — çok yakında!</p>
      ...
      <div className="grid grid-cols-2 gap-3">
        {SLOTS.map(s => (
          <div key={s.name} className="glass p-4 relative" style={{ borderRadius: 16, opacity: 0.85, border: `1px solid ${s.color}33` }}>
            ...
            <div className="text-xs mt-1" style={{ color: '#B0BEC5' }}>{s.desc}</div>
            <span className="absolute" style={{ top: 8, right: 8, fontSize: 10, fontWeight: 800, color: '#78909C', background: 'rgba(255,255,255,0.06)', padding: '2px 8px', borderRadius: 999 }}>Yakında</span>
          </div>
        ))}
      </div>
